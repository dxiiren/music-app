...
a Firebase (Auth + Firestore + Storage) integration specialist}. {Any relevant seniority,
domain, or mindset.}
...
situation: what the project is (a Vue 3 + Vite music-streaming SPA backed by Firebase -
Auth, Firestore, Storage - with a Howler.js audio player), the domain, and WHY this task
...
## 6. Constraints & Rules
- Tech stack: {Vue 3.5 / Vite 6 / plain JS / Tailwind v4 / VeeValidate 4 / Pinia / Firebase 11 /
  Howler 2 / vue-i18n 9, versions from package.json}
...
## 2. Mission
Make the player's seek/progress display update reliably, and explain the root cause
in one paragraph.

## 3. Background & Context
Vue 3 music-streaming app. Playback runs through a Pinia store wrapping a Howler.js `Howl`;
a `requestAnimationFrame` loop copies `seek()`/`duration()` into formatted display state.
Users report the progress bar occasionally shows `NaN%` right after a new song starts.
No repo access - everything you need is below.

## 4. Current State
- Done: playback, pause/resume, and click-to-seek work.
- Failing: for a moment after `newSong()`, `duration()` is `0` and the progress math
  divides by it.
...
## 6. Constraints & Rules
- Plain JS (no TypeScript), Pinia options-style store, no new dependencies.

## 7. Your Task
Return the corrected store actions and a one-paragraph root-cause explanation.
...
- Ported from akmal-resume-website for music-app: same 9-section framework and
  dual-mode delivery; persona, stack constraints, and the worked example adapted to this
  repo (Vue 3 + Vite + plain JS, the Howler-backed player store).
