...
`github.com/dxiiren/music-app`. This repo has no CI and no deploy pipeline —
...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `feat(player): add shuffle to the playlist` or
  `fix(upload): validate audio mime type before upload`. Pick the type/scope from the
...
- <how it was verified: `just lint` result, `just test` result if specs exist,
  manual check against the dev server on :8115 (Firebase-backed features need a real
  config in `src/includes/firebase.js`)>
```
