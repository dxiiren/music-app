...
| --- | -------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Accessibility**          | issue      | Interactive `<div>`/`<span>` with `@click` instead of `<button>`/`<router-link>`; icon-only buttons without `aria-label`; `<img>` without meaningful `alt`; inputs without an associated `<label for>` (the existing VeeValidate auth/upload forms set the pattern); heading order (no jumps). |
...
| 4   | **`v-for` keys**           | issue      | Missing `:key`, `:key="index"` on a list that reorders/filters (the song playlist and comment lists sort/paginate — keys must be `song.id` / `comment.id`), or `v-if` on the same element as `v-for`.                                                                                                                          |
| 5   | **Composable correctness** | issue      | Missing cleanup (`onUnmounted` / `onScopeDispose`) for listeners, intervals, observers; replaced `Howl` instances stopped + unloaded (the player store's `newSong` sets the pattern); `requestAnimationFrame` loops that keep running after playback stops.                                                                                   |
| 6   | **Firebase contract**      | issue      | All Firestore/Auth/Storage calls go through the `src/includes/firebase.js` export bundle (never re-import `firebase/*` in components); paginated song queries keep the `orderBy` + `startAfter(lastDoc)` + `limit` cursor consistent; errors caught and surfaced (not swallowed silently); auth-gated writes check `userLoggedIn` first.              |
...
| 8   | **Security**               | issue      | No `v-html` with untrusted content (XSS); no hardcoded credentials/tokens; uploaded-file checks stay client-side hints only (Firebase Storage rules are the real gate); nothing secret in committed config.                                                                                                 |
...
| 10  | **Tests**                  | suggestion | Existing specs live in `src/components/__tests__/` — for new logic-bearing code (stores, helpers, mappers), suggest a matching `*.spec.js` there rather than growing the untested surface.                                                                                              |
| 11  | **Component design**       | suggestion | `defineProps`/`defineEmits` declared for every prop/event used; oversized components worth splitting; Firestore-fetch logic accumulating in views that belongs in a Pinia store or shared helper.                                                                                                       |
