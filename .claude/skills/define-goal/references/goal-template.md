...
- {Step to bring the world up + how to check it's up + what to do if it's down. e.g. `npm ci`, the
  dev server (`just start` on :8115), a sanity gate (`npx eslint .`). Reference the exact command.}
...
Author Vitest specs so `includes/helper.js`, `stores/player.js`, and
`components/CompositionItem.vue` each have a passing spec covering their real behavior.
Success = `just test` exits 0 with all specs green (existing specs stay green too).
...
| #   | Item                                 | Status |
| --- | ------------------------------------ | ------ |
| 1   | includes/helper.js                   | `TODO` |
| 2   | stores/player.js                     | `TODO` |
| 3   | components/CompositionItem.vue       | `TODO` |
