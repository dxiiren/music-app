...
- What's the per-item verification (`npx eslint .` / `just test` / grep / a manual check on
  :8115) that proves the change is safe?
...
- This repo runs locally only (no CI/CD, no deploy target) - a "deployment" goal here usually
  means the local stack: `just build` green, `just preview` serving `dist/` on :8115, and a
  real Firebase config filled in `src/includes/firebase.js`. State the health-verify that
  proves success.
