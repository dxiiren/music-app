...
4. **Guardrails & locked decisions** - may it `git commit` / `push` / stage? May it open a PR /
   ...
   **locked** (do-not-relitigate)? Any **environment bootstrap** (`npm ci`, the dev server on
   :8115, a sanity gate like `npx eslint .`)?
...
- Ported from akmal-resume-website for music-app - same interrogation -> confirm-until-100%
  -> stop-proof `{topic}-goal.md` flow and the five stop-proof pillars; examples and environment
  bootstrap adapted to this repo (Vue 3 + Vite, plain JS, port 8115, no coverage gate, no deploy).
