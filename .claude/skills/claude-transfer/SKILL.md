...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   ...
   and load http://localhost:8115) to catch drift or undocumented breakage. Building a new
...
- Files: `src/views/HomeView.vue`, `src/stores/player.js`, `src/components/AppPlayer.vue`
...
- Dev server on :8115 was {up/down} at close - re-check
- Firebase config in `src/includes/firebase.js` was {filled/empty} at close - the app
  needs it to mount and load songs
...
- Ported from akmal-resume-website for music-app: pointer-based, read only on
  explicit resume, never auto-loaded; SCOPE GUARD + freshness re-verify. Commands + freshness
  checks adapted to this repo (just recipes, port 8115, the Firebase dependency).
