MCP smoke-test prompts -- one .txt per server, for music-app.
...
FILES (3 server checks)
  ...
  Browser:
    playwright.txt       -> browser automation (needs `just start` for :8115, or about:blank) [turnkey]
  Repo (per-dev):
    github.txt           -> GitHub identity + repo/PR (PAT) [per-dev]

PRECONDITIONS
  - Tool ids are mcp__<server>__<tool> (hyphens kept).
  - playwright(localhost) needs the dev server (`just start`, port 8115).
