...
music-app/
  ...
  dev-dist/                 # PWA dev service-worker output (git-ignored; regenerated by dev server)
...
- The PWA plugin runs in dev (`devOptions.enabled`) and regenerates `dev-dist/` on every dev
  run — the folder is **git-ignored** (and eslint-ignored), so that churn never touches
  `git status` or lint; never track it.
